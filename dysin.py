...
class Spirit(QtGui.QLabel):
	...
	def update(self):
		...
		if(self.stopMoving or self.character.dialog.standFlag or self.testActionMenu.isVisible() or self.character.mood.stopMoving
...
class Dialog(QtGui.QWidget):
	...
	def paintEvent(self, paintEvent):
		...
		painter.drawPixmap(0, 0, Pixmap(iconName).scaledToWidth(self.iconWidth, mode = QtCore.Qt.SmoothTransformation).copy(0, 0, self.iconWidth, self.iconHeight))
...
class MainApplication:
	...
	def settingOnTop(self):
		if(self.onTopAction.isChecked()):
			...
			self.character.spirit.setWindowFlags(QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint | QtCore.Qt.Tool)
			self.character.dialog.setWindowFlags(QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint | QtCore.Qt.Tool)
			self.character.spirit.show()
